...
# To save text entered in the text widget as a new file
def save_as_file():
...
# To save text into an already existing file
def save_files():
    global open_status_name
    if open_status_name:
        save_file = open(open_status_name, 'w')
        save_file.write(text_field.get(1.0, END))
        save_file.close()
        basename = os.path.basename(open_status_name)
        root.title(f'{basename} - ET-Editor')
    else:
        save_as_file()
...
# To cut text from text widget to the clipboard
def cut_text(e):
    global selected
    if e:
...
# To copy text from text widget to the clipboard
def copy_text(e):
    global selected
...
# To paste text from the clipboard into the text widget
def paste_text(e):
    global selected
    if e:
        selected = root.clipboard_get()
...
icon = PhotoImage(file=r'ET Editor\note-taking.png')
root.iconphoto(False, icon)
root.iconbitmap(r'ET Editor\note-taking.ico')
